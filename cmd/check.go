...
func checkAzEnv() ([]string, error) {
	...
	if !ok || os.Getenv("AZURE_DEVOPS_EXT_PAT") == "" {
		...
		return []string{"⚠️", "AZURE_DEVOPS_EXT_PAT", "not set", "env var is required for auth UNLESS using SSO based login (better security ⚡)"}, errors.New("AZURE_DEVOPS_EXT_PAT not set") //nolint:revive // revive: ok to leave constant
	}
